...
function createCountryCard(countryData) {
  ...
  div.innerHTML = `
      <div class="card">
        <div class="card-body">
          ...
          <img src="${
            countryData.flag
          }" class="card-img-top img" alt="country img">
          <p class="card-title capital">Capital: ${countryData.capital}</p>
          <p class="card-title">Region: ${countryData.region}</p>
          <button type="button" class="btn btn-primary btn-weather">Current Weather</button>
  ...
  weatherButton.addEventListener("click", () => {
    fetchWeatherDetails(countryData.alpha3Code).then((weatherData) => {
      if (weatherData) {
